...
if ("seqnames" %in% names(test) & "internalPromoter" %in% names(test)) {
  ...
  test <- test[test$internalPromoter == FALSE | is.na(test$internalPromoter) | is.na(test$seqnames), ]
  ...
  write.table(sorted_data, file = "/home/lucianhu/rna_snakemake/external_promoters.v19.bed", 
